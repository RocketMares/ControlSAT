    ...
    if ($_FILES["archivito"]['error'] != 4) {
        ...
        if(in_array($ext_archivo, $foratos_permitidos)){
            if (move_uploaded_file($nombre_temp_archivo, $path)) {
                ...
                header("location:../Carga_masiva_usuarios_insumo.php?resultado=1");
            }else{
                //El archivo es más pesado de lo permitido en el servidor (10 MB).
                header("location:../Carga_masiva_usuarios_insumo.php?resultado=4");
            }

        }else{
        //el formato del documento no esta permitido.
            header('Location:../Carga_masiva_usuarios_insumo.php?resultado=3');
             
        }
    }else{
        //no se selecciono ningun archivo
        header('Location:../Carga_masiva_usuarios_insumo.php?resultado=2');
    }
