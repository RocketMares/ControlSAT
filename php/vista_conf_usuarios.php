...
class Conf_Users
{
    public function Vista_General()
    {
        ...
        $paginacion = new VistasPlantilla();

        $resultados_por_pagina = 50;
        $universo_datos = $users->Consulta_usuarios_pag_num($_SESSION["ses_id_admin_ing"]);
        $paginas = $universo_datos / $resultados_por_pagina;
        $valor_redondeado = ceil($paginas);

        $pagina = $_GET['usuarios'];
        $nombre_pag =  "usuarios_ingresos";
        $nombre_get = "usuarios";
        $datos_paginacion =
            array(
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_usuarios_pag($_SESSION["ses_id_admin_ing"], $inicio);
        } else {
        ...
        for ($i = 0; $i < count($valores); $i++) {
            if ($valores[$i]["estatus"] == "ACTIVO") {
                ...
                $html[$i] = "<tr>
                                ...
                                <td>
                                    <a class='btn btn-info text-white btn-block btn btn-secondary' data-toggle='tooltip' data-placement='right' title='Editar Usauario' onclick='ConsultarDatosUser_us(" . $valores[$i]["id_empleado_us"] . ")'><i class='fas fa-user-edit'></i></a>
                                    <a class='btn btn-info text-white btn-block btn btn-secondary' data-toggle='tooltip' data-placement='right' title=' Responsiva' href='php/Resp.php?id_usuario=" . $valores[$i]["id_empleado_us"] . "' target='_blank'><i class='fas fa-envelope-open-text'></i></a>
                                </td>
                            </tr>";
            } else {
                $html[$i] = "<tr $inactivos>
    ...
    public function vista_por_nombre()
    {
        ...
        $universo_datos = $users->Consulta_usuarios_nombre_pag_num($_SESSION["ses_id_admin_ing"], $nombre);
        $paginas = $universo_datos / $resultados_por_pagina;
        $valor_redondeado = ceil($paginas);
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_usuarios_nombre_pag($_SESSION["ses_id_admin_ing"], $nombre, $inicio);
        } else {
            $pagina = $pagina - $datos_paginacion['primera_pag'];
            $inicio = ($resultados_por_pagina * $pagina) + 1;
            $valores = $users->Consulta_usuarios_nombre_pag($_SESSION["ses_id_admin_ing"], $nombre, $inicio);
            ...
            for ($i = 0; $i < count($valores); $i++) {
                if ($valores[$i]["estatus"] == "ACTIVO") {
                    ...
                    $html[$i] = "<tr>
                                    ...
                                    <td>
                                        <a class='btn btn-info text-white btn-block btn btn-secondary' data-toggle='tooltip' data-placement='right' title='Editar Usauario' onclick='ConsultarDatosUser_us(" . $valores[$i]["id_empleado_us"] . ")'><i class='fas fa-user-edit'></i></a>
                                        <a class='btn btn-info text-white btn-block btn btn-secondary' data-toggle='tooltip' data-placement='right' title='Imprimir Responsiva' href='php/Resp.php?id_usuario=" . $valores[$i]["id_empleado_us"] . "' target='_blank'><i class='fas fa-envelope-open-text'></i></a>
                                    </td>
                                </tr>";
                } else {
                    $html[$i] = "<tr $inactivos>
    ...
    public function vista_por_nombre_sub()
    {
        ...
        $universo_datos = $users->Consulta_usuarios_nombre_sub_pag_num($_SESSION["ses_id_admin_ing"], $sub, $nombre);
        $paginas = $universo_datos / $resultados_por_pagina;
        $valor_redondeado = ceil($paginas);
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_usuarios_nombre_sub_pag($_SESSION["ses_id_admin_ing"], $sub, $nombre, $inicio);
        } else {
            $pagina = $pagina - $datos_paginacion['primera_pag'];
            $inicio = ($resultados_por_pagina * $pagina) + 1;
            $valores = $users->Consulta_usuarios_nombre_sub_pag($_SESSION["ses_id_admin_ing"], $sub, $nombre, $inicio);
    ...
    public function vista_por_nombre_sub_dep()
    {
        ...
        $sub = $_COOKIE["subadmin"];
        $dep = $_COOKIE["deptos"];
        $universo_datos = $users->Consulta_usuarios_nombre_sub_dep_pag_num($_SESSION["ses_id_admin_ing"], $sub, $dep, $nombre);
        $paginas = $universo_datos / $resultados_por_pagina;
        $valor_redondeado = ceil($paginas);
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_usuarios_nombre_sub_dep_pag($_SESSION["ses_id_admin_ing"], $sub, $dep, $nombre, $inicio);
        } else {
            $pagina = $pagina - $datos_paginacion['primera_pag'];
            $inicio = ($resultados_por_pagina * $pagina) + 1;
            $valores = $users->Consulta_usuarios_nombre_sub_dep_pag($_SESSION["ses_id_admin_ing"], $sub, $dep, $nombre, $inicio);
    ...
    public function vista_por_nombre_sub_dep_per()
    {
        ...
        $universo_datos = $users->Consulta_usuarios_nombre_sub_dep_per_pag_num($_SESSION["ses_id_admin_ing"], $sub, $dep, $perfil, $nombre);
        $paginas = $universo_datos / $resultados_por_pagina;
        $valor_redondeado = ceil($paginas);
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_usuarios_nombre_sub_dep_per_pag($_SESSION["ses_id_admin_ing"], $sub, $dep, $perfil, $nombre, $inicio);
        } else {
            $pagina = $pagina - $datos_paginacion['primera_pag'];
            $inicio = ($resultados_por_pagina * $pagina) + 1;
            $valores = $users->Consulta_usuarios_nombre_sub_dep_per_pag($_SESSION["ses_id_admin_ing"], $sub, $dep, $perfil, $nombre, $inicio);
    ...
    public function vista_por_perfil_SUB_DEP()
    {
        ...
        if ($pagina == 1) {
            $inicio = 1;
            $valores = $users->Consulta_por_perfil_sub_dep_pag_num($_SESSION["ses_id_admin_ing"], $perfil, $sub, $dep, $inicio);
        } else {
            $pagina = $pagina - $datos_paginacion['primera_pag'];
            $inicio = ($resultados_por_pagina * $pagina) + 1;
            $valores = $users->Consulta_por_perfil_sub_dep_pag_num($_SESSION["ses_id_admin_ing"], $perfil, $sub, $dep, $inicio);
